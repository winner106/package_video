...
### Rename the app (one command)

Use the bundled script (Node, pnpm):

```bash
pnpm run rename:app -- --name "Your App Name"
```

This updates:

- `src-tauri/tauri.conf.json` (`productName`, `identifier` ➜ `com.{slug}.app`)
- `package.json` (`name` slug + `productName` if present)
- `src-tauri/Cargo.toml` package name (slug)
- `locales/*` (`app.name`, `titlebar.default`)
- `src-tauri/src/commands/tray.rs` tooltip text
...
Update these files manually if you don't use the script:

### Configuration Checklist

| File                            | Fields to Update                                                               |
| ------------------------------- | ------------------------------------------------------------------------------ |
| `package.json`                  | `name`, `description`, `productName` (if present)                              |
...
This script will use the source image file `app-icon.png` to create all the icons needed for the various platforms.
