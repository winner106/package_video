...
### Disabling the Quick Pane (Template Switch)

If your project doesn't need quick panes, disable it at startup:

- Set `TAURI_ENABLE_QUICK_PANE=0` (also accepts `false` / `off`)
- Or set `QUICK_PANE_ENABLED_DEFAULT = false` in `src-tauri/src/lib.rs`

When disabled, the quick pane window is not created and the global shortcut is not registered.
