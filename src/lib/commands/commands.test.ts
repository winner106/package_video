...
const { registerCommands, getAllCommands, executeCommand } = await import('./registry')
...
describe('Simplified Command System', () => {
  ...
  describe('Command Registration', () => {
    it('registers commands correctly', () => {
      ...
      const sidebarCommand = commands.find(cmd => cmd.id === 'show-left-sidebar' || cmd.id === 'hide-left-sidebar')
    ...
    it('filters commands by availability', () => {
      ...
      const showSidebarCommand = availableCommands.find(cmd => cmd.id === 'show-left-sidebar')
      const hideSidebarCommand = availableCommands.find(cmd => cmd.id === 'hide-left-sidebar')
    ...
    it('filters commands by search term using translations', () => {
      ...
      searchResults.forEach(cmd => {
        ...
        const description = cmd.descriptionKey ? mockT(cmd.descriptionKey).toLowerCase() : ''
        const matchesSearch = label.includes('sidebar') || description.includes('sidebar')
