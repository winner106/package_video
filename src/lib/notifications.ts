...
interface NotificationOptions {
  ...
  duration?: number
  /** Stable ID to deduplicate/update a toast */
  id?: string
}
 ...
 * ```
 */
export async function notify(title: string, message?: string, options: NotificationOptions = {}): Promise<void> {
  const { type = 'info', native = false, duration = DEFAULT_TOAST_DURATION, id } = options
  ...
  try {
    if (native) {
      ...
      const toastOptions = duration !== undefined ? { duration } : { duration: DEFAULT_TOAST_DURATION }
      if (id) {
        Object.assign(toastOptions, { id })
      }
...
export const notifications = {
  ...
  success: (title: string, message?: string, native?: boolean) => notify(title, message, { type: 'success', native }),

  /** Show error notification */
  error: (title: string, message?: string, native?: boolean) => notify(title, message, { type: 'error', native }),

  /** Show info notification */
  info: (title: string, message?: string, native?: boolean) => notify(title, message, { type: 'info', native }),

  /** Show warning notification */
  warning: (title: string, message?: string, native?: boolean) => notify(title, message, { type: 'warning', native }),
}
