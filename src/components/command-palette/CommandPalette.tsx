...
import { formatShortcutForDisplay } from '@/lib/commands/shortcut-utils'
import {
  CommandDialog,
  CommandInput,
  CommandList,
        ...
        {Object.entries(commandGroups).map(([groupName, groupCommands]) => (
          <CommandGroup key={groupName} heading={getGroupLabel(groupName)}>
            {groupCommands.map(command => (
              <CommandItem key={command.id} value={command.id} onSelect={() => handleCommandSelect(command.id)}>
                {command.icon && <command.icon className="mr-2 h-4 w-4" />}
                <span>{t(command.labelKey)}</span>
                {command.descriptionKey && (
                  <span className="ml-auto text-xs text-muted-foreground">{t(command.descriptionKey)}</span>
                )}
                {command.shortcut && <CommandShortcut>{formatShortcutForDisplay(command.shortcut)}</CommandShortcut>}
              </CommandItem>
            ))}
          </CommandGroup>
        ))}
